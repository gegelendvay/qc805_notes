...
Example: One-time pad (Vernam cipher, 1926): Alice and Bob share a secret key $S_A=S_B \in \{0,1\}^*$ without Eve knowing.
Alice encrypts the message $m \in \{0,1\}^* (c)_i = m_i \oplus (S_A)_i$
...
\begin{enumerate}
    ...
    \item Key is not re-used
    \item Key must be completely secret
\end{enumerate}

mathematical properties of the key:
Ideally:
\begin{enumerate}
    \item Correctness $S_A=S_B$
    \item Randomness $P[S=S] = 2^{-n}$
    \item Eve has no information about the key
\end{enumerate}

More realistically:
$P[S_A \neq S_B] < \epsilon$
Close to uniformly random, individual bits are independent
If Eve interferes with key generation, then we do not produce a key
...
\paragraph{Quantum Transmission}
Alice chooses two bit-strings at random: $a,b \in \{0,1\}^*$ where $a$ is the key and $b$ is the encoding basis.

Alice prepares qubit states $\ket{\psi_i}$ by encoding $a_i$:
\begin{enumerate}
    \item In the computational basis, $\ket{0}$ and $\ket{1}$
    \item Diagonal basis $\ket{+}$, $\ket{-}$
\end{enumerate}

Alice sends them across the quantum channel

Bob chooses $b'$ at random and measures $\ket{\psi_i}$ in basis $b_i$: $a'_i$ = measurement of $\ket{\psi_i}$ in basis $b'_i$
If $b'_i = b_i$ then $a'_i = a_i$ else if $b'_i \neq b_i$ then $a'_i$ is uniformly random and gets discarded

If $b'_i = b_i$: $0 \to \ket{0}, 1 \to \ket{1}$, then in a computational basis state, $P_{\psi}(0) = |\bra{0}\ket{\psi}|^2$ and $P_{\psi}(1) = |\bra{1}\ket{\psi}|^2$
$P_{\ket{0}}(0) = 1$ and $P_{\ket{0}}(1) = 0$, meaning $a'_i = 0$

if $b'_i \neq b_i$, then $a_i = 0$
$a_i = 0, b_i = 1 \to \ket{\psi_i} = \ket{+}$
$P_{\ket{+}}(0) = \frac{1}{2} and P_{\ket{+}}(1) = \frac{1}{2}$
